...
def get_sentences(data):
    ...
    for document in data:
        ...
        sentences = [" ".join(sentence) if type(sentence) == tuple else sentence for sentence in sentences]
        all_sentences.append(sentences)
        all_sentences.append(["\n\n"])
    final_sentences = []
    for sentences in all_sentences:
        final_sentences += sentences
    print("Sentences collected. Writing sentences...")
...
def main():
    ...
    articles = get_articles(ids=swo_ids)
    print(f"Got {len(articles)} articles")
    get_sentences(data=articles)
