    ...
    def update_segments(self, repl_char="=", other_label="O"):
        ...
        while i < len(tmp_txt):
            if tmp_txt[i] == repl_char:
                ...
                segment.sentences = utils.tokenize(text=segment.text, punct=False)
...
class DataLoader:
    ...
    def load(self, filename="kasteli.json"):
        ...
        with open(join(self.resources_folder, "documents.pkl"), "wb") as f:
            pickle.dump(docs, f)
        return docs
    ...
    def _create_document(self, doc):
        ...
        document.sentences = utils.tokenize(text=document.content, punct=False)
    ...
    def load_adus(self, do_oversample=False):
        ...
        if do_oversample:
            des_lbl_count = {"B-major_claim": 1000, "I-major_claim": 1000, "B-claim": 1000, "I-claim": 1000,
                             "B-premise": 1000,
                             "I-premise": 1000}
            from preprocessing import DataUpSampler
            upsampling = DataUpSampler(app_config=self.app_config)
            upsampling.oversample(task_kind="adu", file_kind="train", total_num=des_lbl_count)

    # **************************** Create relations and stance csv files *******************************
    def load_relations(self, do_oversample=False):
        relations, stances = self._get_relations()
        self._save_rel_df(rel_list=relations, folder="rel", filename="train_rel.csv")
        self._save_rel_df(rel_list=stances, folder="stance", filename="train_stance.csv")
        if do_oversample:
            # TODO oversampling with dynamic total num
            from preprocessing import DataUpSampler
            upsampling = DataUpSampler(app_config=self.app_config)
            upsampling.oversample(task_kind="rel", file_kind="train", total_num=8705)
            upsampling.oversample(task_kind="stance", file_kind="train", total_num=289)
    ...
    def load_similarities(self, do_oversample=False):
        ...
        for index, row in df.iterrows():
            ...
            row_counter += 1
        output_filepath = join(data_path, "train_sim.csv")
